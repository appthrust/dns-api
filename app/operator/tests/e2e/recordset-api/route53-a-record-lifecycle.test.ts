...
test(
  "a Route 53 A RecordSet follows the Zone lifecycle",
  async (s) => {
    ...
    s.when("a RecordSet is created by the application");
    ...
    s.then("the Route 53 A record set exists");
    await s.exec(
      {
        do: async () => {
          await assertARecordSetExists(hostedZoneID, domainName);
        },
      },
      waitForRoute53,
    );
    ...
    s.then("the Route 53 A record set is gone");
    await s.exec(
      {
        do: async () => {
          await assertARecordSetAbsent(hostedZoneID, domainName);
        },
      },
      waitForRoute53,
    );
...
async function assertARecordSetExists(hostedZoneID: string, domainName: string) {
  const record = await getARecordSet(hostedZoneID, domainName);
...
async function assertARecordSetAbsent(hostedZoneID: string, domainName: string) {
  const record = await getARecordSet(hostedZoneID, domainName);
  expect(record).toBeUndefined();
}

async function getARecordSet(hostedZoneID: string, domainName: string) {
