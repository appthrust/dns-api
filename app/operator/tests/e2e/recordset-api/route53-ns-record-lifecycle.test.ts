...
test(
  "a Route 53 NS RecordSet follows the Zone lifecycle",
  async (s) => {
    ...
    s.when("a delegated NS RecordSet is created by the application");
    ...
    s.then("the Route 53 delegated NS record set exists");
    await s.exec(
      {
        do: async () => {
          await assertNSRecordSetExists(hostedZoneID, domainName);
        },
      },
      waitForRoute53,
    );
    ...
    s.then("the Route 53 delegated NS record set is gone");
    await s.exec(
      {
        do: async () => {
          await assertNSRecordSetAbsent(hostedZoneID, domainName);
        },
      },
      waitForRoute53,
    );
...
async function assertNSRecordSetExists(hostedZoneID: string, domainName: string) {
  const record = await getNSRecordSet(hostedZoneID, domainName);
  expect(record?.Name).toBe(`delegated.${domainName}.`);
  expect(record?.Type).toBe("NS");
  expect(record?.TTL).toBe(300);
  const values = record?.ResourceRecords?.map((item) => parseNSRecordValue(item.Value ?? ""));
  expect(values?.sort()).toEqual([...nsNameServers].sort());
}

async function assertNSRecordSetAbsent(hostedZoneID: string, domainName: string) {
  const record = await getNSRecordSet(hostedZoneID, domainName);
  expect(record).toBeUndefined();
}

async function getNSRecordSet(hostedZoneID: string, domainName: string) {
  const recordName = `delegated.${domainName}.`;
  ...
  if (record?.Name !== recordName || record.Type !== "NS") {
    return undefined;
  }
  return record;
}

function parseNSRecordValue(value: string): string {
  const nameServer = value.trim().replace(/\.$/, "");
  expect(nameServer).not.toBe("");
  return nameServer;
}
