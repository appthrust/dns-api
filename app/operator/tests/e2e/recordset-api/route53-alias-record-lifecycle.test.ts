...
test(
  "a Route 53 ALIAS RecordSet follows the Zone lifecycle",
  async (s) => {
    ...
    s.when("a target A RecordSet is created by the application");
    ...
    s.then("the target RecordSet reports Accepted=True and Programmed=True");
    await assertRecordSetProgrammed(app, "target");

    s.when("an ALIAS RecordSet is created by the application");
    await app.apply<RecordSet>({
      apiVersion: "dns.appthrust.io/v1alpha1",
      kind: "RecordSet",
      metadata: { name: "alias" },
    ...
    s.then("the ALIAS RecordSet reports Accepted=True and Programmed=True");
    await assertRecordSetProgrammed(app, "alias");

    s.then("the Route 53 ALIAS record set exists");
    await s.exec(
      {
        do: async () => {
          await assertAliasRecordSetExists(hostedZoneID, domainName);
        },
      },
      waitForRoute53,
    );

    s.when("the ALIAS RecordSet is deleted by the application");
    ...
    s.then("the Route 53 ALIAS record set is gone");
    await s.exec(
      {
        do: async () => {
          await assertARecordSetAbsent(hostedZoneID, `alias.${domainName}.`);
        },
      },
      waitForRoute53,
    );

    s.when("the target RecordSet is deleted by the application");
    await app.delete<RecordSet>(
      {
        apiVersion: "dns.appthrust.io/v1alpha1",
        kind: "RecordSet",
    ...
    s.then("the Route 53 target record set is gone");
    await s.exec(
      {
        do: async () => {
          await assertARecordSetAbsent(hostedZoneID, `target.${domainName}.`);
        },
      },
      waitForRoute53,
    );
...
async function assertRecordSetProgrammed(
  namespace: Namespace,
  name: string,
) {
...
async function assertAliasRecordSetExists(hostedZoneID: string, domainName: string) {
  const record = await getARecordSet(hostedZoneID, `alias.${domainName}.`);
  expect(record?.Name).toBe(`alias.${domainName}.`);
...
async function assertARecordSetAbsent(hostedZoneID: string, recordName: string) {
  const record = await getARecordSet(hostedZoneID, recordName);
  expect(record).toBeUndefined();
}

async function getARecordSet(hostedZoneID: string, recordName: string) {
