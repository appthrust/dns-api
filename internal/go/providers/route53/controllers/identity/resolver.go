...
func validateAWSRegionEndpoint(_ context.Context, region string) error {
	if errs := validation.IsDNS1123Label(region); len(errs) > 0 {
		return fmt.Errorf("region must be a valid AWS region label: %s", strings.Join(errs, "; "))
	}
	return nil
}
